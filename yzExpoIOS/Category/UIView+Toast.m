...
NSString * CSToastPositionDown      = @"CSToastPositionDown";
...
- (CGPoint)cs_centerPointForPosition:(id)point withToast:(UIView *)toast {
    ...
    if([point isKindOfClass:[NSString class]]) {
        ...
        } else if([point caseInsensitiveCompare:CSToastPositionDown] == NSOrderedSame) {
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height * 2 / 3);
        }
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.defaultPosition = CSToastPositionDown;
    }
    return self;
}
