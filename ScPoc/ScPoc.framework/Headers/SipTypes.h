...
/** Raise Error exception */
#define SCSIP_RAISE_ERROR3(status, op, reason) \
    do { \
        NSLog(@"ERROR method: %@, status=%d %@, File %s.%s(line:%d)", \
        op, status, reason, __FILE__, __FUNCTION__, __LINE__); \
    } while (0)
//#define SCSIP_RAISE_ERROR3(status, op, reason) \
//    do { \
//        NSError *err = [[NSError alloc] initWithDomain:NSCocoaErrorDomain code:NSURLErrorUnknown \
//        userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:@"method: %@, status=%d %@, File %s.%s(line:%d)", \
//        op, status, reason, __FILE__, __FUNCTION__, __LINE__] }]; \
//        @throw err; \
//    } while (0)
